%% PRÁCTICA 13.10. Análisis tiempo‑frecuencia mediante la Transformada Continua Wavelet (CWT)
...
% Con cualquiera de las dos líneas siguientes se obtuvo lo mismo y no influyó en nada.
tic
...
%Si no se emplea la instrucción anterior, la siguiente instrucción no funcionaría:
...
tic
figure
% Uso de la función SIN PARÁMETROS DE SALIDA:
% Las dos siguientes líneas de código produjeron muy malos resultados:
...
% Uso de la función CON PARÁMETROS DE SALIDA: da mejores resultados:
[P, F, T]=pspectrum(yt1,Fs,'FrequencyLimits',[0 100],'FrequencyResolution',1,'Leakage',0.85,'spectrogram','Reassign',true);
mesh(T,F,P) % produjo resultados muy buenos
colormap parula
title('ESPECTROGRAMA REASIGNADO EN FRECUENCIA')
xlabel('Tiempo'), ylabel('F(Hz)'), axis tight
view(2)
toc
...
% MEJOR RESOLUCION EN FRECUENCIA: ventana = 20% de la longitud de la señal
tic
window_F= blackmanharris(fix(length(yt1)/5));
[d_F, f_F, t_F] = fsst(yt1,Fs,window_F);
figure
contour(t_F,f_F,abs(d_F));
title('FSST con longitud de ventana = 20% de la longitud de la señal')
xlabel('t(sec)'), ylabel('F(Hz)')
toc
...
figure
%función CWT con tipo de wavelet Morse, con parámetros por defecto:
% cwt(yt1,Fs,'morse'); 
% title(' CWT (Wavelet Morse), Parámetros por defecto [3,60]');
%función CWT con parámetros que obtienen mejores resultados [12,440]:
...
figure
%función CWT con tipo de wavelet Morlet:
cwt(yt1,Fs,'amor'); 
title(' CWT (Wavelet MORLET)');
%Gráfica "Fine Scale Coefficients": la función se emplea con parámetros de salida
wt = cwt(yt1,Fs,'amor');
figure
plot(t,10*log10(abs(wt(1,:)).^2)); 
xlabel('Tiempo (s)'); ylabel('Magnitud'); grid on;
title('Diagrama "Fine Scale Coefficients');
