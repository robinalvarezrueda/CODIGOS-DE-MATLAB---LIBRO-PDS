%% PRÁCTICA 13.2. Obtención y análisis del espectrograma en 2D y 3D
...
%%  4.- ANÁLISIS EN TIEMPO-FRECUENCIA
% 4.1 ESPECTROGRAMA SIN PARÁMETROS DE SALIDA: 
% La ventana deslizante será de longitud del 5% de la longitud total
L=fix(0.05*length(yt1));
% Tipo de ventana de longitud L:
%ventana= rectwin (L);
ventana= hann (L);
%ventana= blackmanharris (L);
% Traslape del 50%:
% El número de muestras de traslape que debe ser entero:
noverlap = fix(length(ventana)/2);
FACTOR = 256;
nFFT_ventana = 2^ceil(log2(length(ventana)))*FACTOR;
figure
subplot(2,2,1)
spectrogram(yt1, ventana,noverlap, nFFT_ventana, Fs,'yaxis');
title('Espectrograma del FENÓMENO 1 ')
subplot(2,2,2)
spectrogram(yt2, ventana, noverlap, nFFT_ventana, Fs,'yaxis');
title('Espectrograma del FENÓMENO 2 ')
% 4.2 ESPECTROGRAMA CON PARÁMETROS DE SALIDA y empleando la función contour
% FENÓMENO 1:
subplot(2,2,3)
[P1,f1,t1]=spectrogram(yt1,ventana,noverlap,nFFT_ventana,Fs,'yaxis');
%muy importante para detectar componentes pequeñas:
contour(t1,f1,10*log10(abs(P1))); 
% Nota: si no se aplica logaritmo, no se podrá apreciar componentes pequeñas.
xlabel('t(seg.)'), ylabel('f(Hz)')
title('Espectrograma del FENÓMENO 1 -- CONTOUR')
subplot(2,2,4)
% FENÓMENO 2:
[P2,f2,t2]=spectrogram(yt2,ventana,noverlap,nFFT_ventana,Fs,'yaxis');
%muy importante para detectar componentes pequeñas:
contour(t2,f2,10*log10(abs(P2)));
% No se podrá apreciar componentes pequeñas:
%contour(t2,f2,(abs(P2)));
xlabel('t(seg.)'), ylabel('f(Hz)')
title('Espectrograma del FENÓMENO 2 -- CONTOUR')
%% DIAGRAMA TIEMPO-FRECUENCIA EN 3D empleando la función mesh:
figure
subplot(2,1,1)
mesh(t1,f1,abs(P1)); % Grafica del espectro en 3D como malla
view(160,40); % Change 3-D plot view
title('Comportamiento en tiempo-frecuencia en 3D - FENÓMENO 1')
xlabel('Tiempo (seg)')
ylabel('F (Hz)')
subplot(2,1,2)
mesh(t2,f2,abs(P2)); % Grafica del espectro en 3D como malla
view(160,40); % Change 3-D plot view
xlabel('Tiempo (seg)')
ylabel('F (Hz)')
title('Comportamiento en tiempo-frecuencia en 3D - FENÓMENO 2')
